...
class Radio:
    def __init__(
        ...
        self.f_offset = int(
            f_offset
        )  # default offset by 250 Khz to avoid sdr center spike
        ...
        self.N = round(
            self.f_sps * self.buffer_time
        )  # number of samples to collect per buffer

        # initialize multiprocessing processes
        self.sample_process = SampleProcess(
            self.f_sps, self.f_c, self.f_offset, self.N, sample_queue, exitFlag
        )
        self.extraction_process = ExtractionProcess(
            self.f_sps,
            self.f_offset,
            self.f_audiosps,
            sample_queue,
        ...
        self.output_queue = multiprocessing.Queue(
            25
        )  # for other programs to use audio samples. max size is 25 to avoid memory overuse if output is not being used.
    ...
    def run(self):
        ...
        while not exitFlag.is_set():
            try:
                audio = audio_queue.get(block=True, timeout=1.0)
    ...
    def cleanup(self):
        ...
        self.stream.stop()
        print("Joining extraction process")
        self.extraction_process.join(1)
        print("Joining sample process")
        self.sample_process.join(1)
        print("done")
...
class SampleProcess(multiprocessing.Process):
    ...
    def run(self):
        asyncio.run(
            self.stream_samples(self.buffer_length, self.f_sps, self.f_c, self.f_offset)
        )
...
class ExtractionProcess(multiprocessing.Process):
    def __init__(
        self, f_sps, f_offset, f_audiosps, sample_queue, audio_queue, exit_flag
    ):
        multiprocessing.Process.__init__(self)
        self.f_sps = f_sps
        self.f_offset = f_offset
    ...
    def process_signal(self, filteredsignal, f_sps, f_audiosps):
        ...
        derivtheta_padded = np.pad(
            derivtheta.astype(float),
            (0, dsf - derivtheta.size % dsf),
            mode="constant",
...
def main(
    f_sps: float=1.0 * 256 * 256 * 16,
    f_audiosps: int=48000,
    f_c: float=94.9e6,
    f_offset: float=250e3,
    buffer_time: float=1.0,
):
    r = Radio(f_sps, f_audiosps, f_c, f_offset, buffer_time)
    r.run()
